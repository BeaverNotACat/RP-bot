...
def form_money_transfer_emmed(character, amount, target):
    ...
    embed.add_field(name='Статус - подтверждена',
                    value=f'{target} получает {amount} анх от персонажа {character} ')
    return embed


def form_error_emmed(message):
    embed = discord.Embed(color=0xD22A00, title='Возникла ошибка!')
    embed.add_field(name='Операция не выполнена по причине:',
                    value=f'{message}')
    return embed
